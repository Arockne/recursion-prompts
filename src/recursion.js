...
var flatten = function(array) {
  ...
  array.forEach(function(element) {
    //if the element in the input array is an array
    if (Array.isArray(element)) {
      //insert the value of that array through recursion utilizing spread syntax to grab each element
...
//I
  //string
//O
  //an objec with the count of each letter
var letterTally = function(str, obj) {
  ...
  //if the obj is not defined, assign with an empty obj
  obj = obj || {};
  //store the calling of the function with the argument of removeing the first element of the string
  var result = letterTally(str.slice(1), obj);
  //if the obj does not have the letter as a key
  if (result[str[0]] === undefined) {
    //create the property with the value of one
    result[str[0]] = 1;
    //otherwise
  } else {
    //increment the current value of the property
    result[str[0]]++;
  }
  //return the obj
  return result;
...
var compress = function(list) {
  ...
  //store the call to the function with an argument of removing the first element
  var result = compress(list.slice(1));
  
  //if the current element is not equal to the next element in the callstack
  if (result[0] !== list[0]) {
